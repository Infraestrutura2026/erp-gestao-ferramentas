 ...
 * • Integração com módulos: config, utils, estoque, ferramentas, indicadores
...
const app = {
  ...
  /* ── Inicialização ── */
  async init() {
    console.log('[APP] Iniciando sistema v' + (CONFIG?.VERSAO || '2.2.0') + '...');
    this._initTheme();
    this._renderLayout();
    ...
    if (!configUI.hasValidUrl()) {
      console.warn('[APP] URL do Sheets não configurada. Redirecionando para Config.');
  ...
  _renderLayout() {
    ...
    const versao = CONFIG?.VERSAO || '2.2.0';
          ...
          <header class="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30">
            ...
            <div class="flex items-center gap-3 shrink-0">
              ...
              <div class="text-right hidden md:block">
                <p class="text-[10px] text-slate-400 leading-tight">v${versao}</p>
                <p class="text-[10px] text-amber-600 font-semibold leading-tight">Polícia Penal</p>
              </div>
              <div class="w-8 h-8 rounded-full bg-slate-800 text-amber-400 flex items-center justify-center text-xs font-bold border-2 border-amber-500 shrink-0">PP</div>
            </div>
          </header>
  ...
  _renderMiniTable(items) {
    if (!items.length) {
      return '<p class="text-sm text-slate-400 text-center py-6">Nenhum registro no momento.</p>';
    }
    return `
          ...
          <tbody class="divide-y divide-slate-100">
            ${items.map(i => `
              <tr class="hover:bg-slate-50/60 transition">
                <td class="py-2 pr-3">
                  <p class="font-medium text-slate-800">${utils.escapeHtml(i.nome || i.item || '—')}</p>
                  <p class="text-xs text-slate-500">${utils.escapeHtml(i.local || '')}</p>
                </td>
                <td class="py-2 text-right whitespace-nowrap">${utils.categoriaBadge(i.categoria)}</td>
                <td class="py-2 pl-3 text-right font-mono text-xs ${(parseFloat(i.quantidadeAtual)||0)===0?'text-red-500':'text-slate-600'}">${parseFloat(i.quantidadeAtual) || 0}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  _isFerramentaQuick(item) {
    ...
    return cat.includes('ferramenta') || cat.includes('manual') || cat.includes('eletrica')
      || nome.includes('ferramenta') || nome.includes('furadeira') || nome.includes('serra')

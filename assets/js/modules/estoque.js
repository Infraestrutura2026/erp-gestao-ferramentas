...
const estoqueModule = {
  ...
  async confirmMovimento(isEntrada) {
    ...
    if (qtd <= 0) {
      ...
      return app.showToast('Informe uma quantidade válida.', 'error');
    }

    const item = app.data.estoque.find(x => x._rowIndex == row);
    if (!item) {
      this._processing = false;
      return app.showToast('Item não encontrado.', 'error');
    }
    ...
    if (!isEntrada && novo < 0) {
      this._processing = false;
      return app.showToast('Saldo insuficiente para saída.', 'error');
    }

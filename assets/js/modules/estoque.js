...
const estoqueModule = {
      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex flex-wrap items-center gap-2">
            <div class="relative">
              ...
              <input id="estoqueSearch" type="text" placeholder="Buscar item..." 
                class="pl-8 pr-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-800 focus:border-blue-800 w-64"
                oninput="estoqueModule.filtrar()">
            </div>
            <select id="estoqueFiltroCategoria" onchange="estoqueModule.filtrar()"
              class="px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-800">
          ...
          <button onclick="estoqueModule.abrirModalAdicionar()" 
            class="px-4 py-2 text-sm bg-blue-900 text-white rounded-lg hover:bg-blue-800 shadow shadow-blue-900/20 transition">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div id="estoqueEmpty" class="hidden p-8 text-center text-slate-400">
  ...
  renderRows(items) {
    if (!items.length) {
      return `<tr><td colspan="7" class="px-4 py-8 text-center text-slate-400">Nenhum item cadastrado.</td></tr>`;
    }
    return items.map(item => {
      ...
      let statusBadge = '';
      if (qtd === 0) {
        statusBadge = `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-700 border border-red-200">✕ ESGOTADO</span>`;
      } else if (qtd <= min) {
        statusBadge = `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">⚠ CRÍTICO</span>`;
      } else {
        statusBadge = `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-200">✓ OK</span>`;
      }

      const catBadge = utils.categoriaBadge(item.categoria);

      return `
        <tr class="border-b border-slate-100 hover:bg-slate-50/60 transition" data-id="${utils.escapeHtml(item.id)}">
    ...
    const html = `
      <div class="space-y-4">
        <div>
          ...
          <input id="inpNome" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Categoria</label>
            <select id="inpCategoria" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
              <option value="">Selecione...</option>
              ${catOptions}
              <option value="__nova__">+ Nova categoria...</option>
            </select>
            <input id="inpCategoriaNova" type="text" placeholder="Digite nova categoria" class="hidden w-full mt-2 border border-slate-300 rounded-lg px-3 py-2 text-sm">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Local</label>
            <input id="inpLocal" type="text" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
        </div>
        <div class="grid grid-cols-3 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Qtd. Atual</label>
            <input id="inpQtd" type="number" min="0" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Qtd. Mínima</label>
            <input id="inpMin" type="number" min="0" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Unidade</label>
            <input id="inpUnidade" type="text" value="un" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
        </div>
      </div>
    `;
    ...
    if (app.isSheetsConfigured()) {
      try {
        ...
        if (res.success) {
          ...
          app.showToast('Erro ao salvar: ' + (res.error || 'desconhecido'), 'error');
          return;
        }
      } catch (e) {
        app.showToast('Erro de conexão: ' + e.message, 'error');
        return;
      }
    }
  ...
  editar(id) {
    ...
    const html = `
      <div class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Nome do Item</label>
          <input id="editNome" type="text" value="${utils.escapeHtml(item.nome || item.item || '')}" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Categoria</label>
            <select id="editCategoria" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
              ${catOptions}
              <option value="__nova__">+ Nova categoria...</option>
            </select>
            <input id="editCategoriaNova" type="text" placeholder="Nova categoria" class="hidden w-full mt-2 border border-slate-300 rounded-lg px-3 py-2 text-sm">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Local</label>
            <input id="editLocal" type="text" value="${utils.escapeHtml(item.local || '')}" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
        </div>
        <div class="grid grid-cols-3 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Qtd. Atual</label>
            <input id="editQtd" type="number" min="0" value="${parseFloat(item.quantidadeAtual) || 0}" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Qtd. Mínima</label>
            <input id="editMin" type="number" min="0" value="${parseFloat(item.quantidadeMinima) || 0}" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Unidade</label>
            <input id="editUnidade" type="text" value="${utils.escapeHtml(item.unidade || 'un')}" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-800">
          </div>
        </div>
      </div>
    `;
  ...
  async atualizar() {
    ...
    if (app.isSheetsConfigured()) {
      try {
        ...
        if (!res.success) {
          app.showToast('Erro ao atualizar Sheets: ' + (res.error || ''), 'error');
          return;
        }
      } catch (e) {
        app.showToast('Erro de conexão: ' + e.message, 'error');
